...
const client = redis.createClient(process.env.REDISCLOUD_URL || 6379)
...
app.get("/ripley", (req, res) => {
  return res
    .status(200)
    .send({ message: "ripley endpoint is working" });
})
...
app.use("/rediscleanall", (req, res) => {
  client.flushall('ASYNC', res.status(200).send({ message: "Deleted Successfully" }))
})
